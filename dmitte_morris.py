...
figname = 'figures/morris/S1_HTO.svg'
Si = analyze(problem, param_values, np.array(Y)[:, 3], print_to_console=True, num_levels=4, num_resamples=100)
df = Si.to_df()
df: pd.DataFrame
fig, ax = plt.subplots(figsize=(4, 3))
ax.scatter(df['mu_star'], df['sigma'], s=10)
labels = ['UU', 'TEMP', 'PAR', 'RAIN', 'RH', 'PVA', 'SW1', 'SW2', 'SW3']
for i, label in enumerate(labels):
    plt.annotate(label, (df['mu_star'][i], df['sigma'][i]), textcoords="offset points", xytext=(0, 5), ha='center', fontsize=6)
ax.set_xlabel(r'$\mu*\;/\;\mathrm{(Bq/m^2)}$')
ax.set_ylabel(r'$\sigma\;/\;\mathrm{(Bq/m^2)}$')
ax.set_ylim(0.28e4, 1.15e4)
fig.savefig(figname, format='svg')
# %%
figname = 'figures/morris/A_HTO.svg'
